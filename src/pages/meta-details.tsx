...
import { useLoadRelated, useLoadSeasonDetails, useMeta } from '../service/catalog/provider';
...
export default function MetaDetails() {
  ...
  // Find the current item's watch progress
  const watchProgress = React.useMemo(() => {
    if (!meta || !continueWatchingItems.length) return undefined;
    
    const currentItem = continueWatchingItems.find(item => 
      item.meta.id === meta.id || 
      item.meta.name === meta.name
    );
    
    return currentItem?.progressPercentage;
  }, [meta, continueWatchingItems]);
  ...
  return (
    ...
    >
      <MetaHeader meta={meta} onPlay={() => handlePlay()} watchProgress={watchProgress} />
      ...
      <div className="container mx-auto relative z-20">
        ...
        >
          <MetaInfo meta={meta} onPlay={() => handlePlay()} watchProgress={watchProgress} />
        </motion.div>
